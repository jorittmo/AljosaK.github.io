---
title: 'Hands-On ML - Project 2: Image classification (fashion-MNIST) using maximum
  likelihood'
...
I contains 60k training and 10k testing images of 28x28 pixels. The labels
...
In the code above we do two important things. First, since the observations are
...
Therefore we transform them into vectors, each row in the dataset representing
...
have observed? In this case we have 10 $28*28 = 784$ dimensional distributions,
...
(non-invertible). Look for example at the eigen values of covariance matrix for
...
Note that when calculating the pseudo determinant I can not
take the product of all eigen values that were strictly > 0. This
...
anyway. Here I choose the threshold 0.5 for including an eigen
...
singularity I could have used taken the log of the inbuilt function for the
multivariate Gaussian density function, like so: 
`ll = np.log(multivariate_normal.pdf(x, mean = mu, cov = Sigma))`. However, this
function does not allow for using the precision directly and would not calculate
...
Then I define a function that call the above function and calculates the
...
So, for example, true images of tshirts gets misclassified tshirts and shirts
gets misclassified as tshirts, but not really to the same extent. We 
can also see that several classes are categorised as shirt as a large portion of 
...
```{python}
for i in range(10):
    print(class_names[i], np.mean(train_y[train_y == i] == train_pred[train_y == i]))
```
...
is better than the full model. This might indicate that the full model slightly
overfit the training data. 

So, to sum up: in this blog post I have shown how you can create an image classification
model without using really fancy techniques that even performs fairly well given that
